...
public class AuthzCodeTokenProvider extends OAuth2AccessTokenSupport implements AccessTokenProvider {
	...
	private MultiValueMap<String, String> getParametersForTokenRequest(AuthorizationCodeResourceDetails resource,
		...
		if ( ( preservedState == null ) && (request.getStateKey() != null ) )
		{
			String resourceRedirect = resource.getRedirectUri(request);
			int index = resourceRedirect.lastIndexOf("?state=");
			// todo: remove the state and add the rest if any
			if (index > 0 )
			{
				resourceRedirect = resourceRedirect.substring(0,  index);
			}
			request.setPreservedState(resourceRedirect);
			preservedState = request.getPreservedState();
		}
