...
public class VmidentityScimGroupMembershipManager implements ScimGroupMembershipManager {
    ...
    public List<ScimGroupMember> query(String filter) {
        logger.debug(String.format("not yet implemeted query: filter'%s'", filter));
    ...
    public List<ScimGroupMember> query(String filter, String sortBy, boolean ascending) {
        logger.debug(
           String.format("not yet implemeted query: filter'%s'n sortby='%s', acsending='%s'",
           filter, sortBy, ascending? "true" : "false"));
        throw new UnsupportedOperationException();
    }

    @Override
    public int delete(String filter) {
        logger.debug(String.format("not yet implemeted delete by filter: filter'%s'", filter));
        throw new UnsupportedOperationException();
    }
            ...
            throws ScimResourceNotFoundException, MemberAlreadyExistsException
    {
        try
        {
            ...
            {
                logger.error("Cannot modify group membership for grouops in non-system domain. " + groupId);
                throw new InvalidScimResourceException("Cannot modify group membership for grouops in non-system domain.");
            }

            Principal idmPrincipal = null;
            if (member.getType() == ScimGroupMember.Type.USER)
            {
                idmPrincipal = this._idmClient.findUser(tenant, member.getMemberId());
                if ( idmPrincipal != null )
                {
                    this._idmClient.addUserToGroup(tenant, idmPrincipal.getId(), group.getName());
                }
                else
                {
            ...
            {
                idmPrincipal = this._idmClient.findGroup(tenant, member.getMemberId());
                if ( idmPrincipal != null )
                {
                    this._idmClient.addGroupToGroup(tenant, idmPrincipal.getId(), group.getName());
                }
                else
                {
            ...
            else
            {
                throw new InvalidArgumentException(
                    String.format("Invalid ScimMemberType '%s'", member.getType()));
            }
        ...
        catch(InvalidPrincipalException ex)
        {
            ...
            throw new ScimResourceNotFoundException(
                String.format("Scim resource not found '%s'.", ex.getPrincipal()) );
        }
        catch (Exception ex)
        {
            logger.debug(String.format("addMember '%s', '%s' failed", groupId, member.getMemberId()), ex);
            throw new IllegalStateException(
                String.format("addMember '%s', '%s' ", groupId, member.getMemberId()), ex);
        }
    }
    ...
    {
        logger.debug(
            String.format("not supported yet getMembers '%s' '%s' '%s' ",
               groupId, filter,
    ...
    public List<ScimGroupMember> getMembers(String groupId, Role permission) throws ScimResourceNotFoundException
    {
        try
        {
            ...
            if ( permission == ScimGroupMember.Role.MEMBER )
            {
                ...
                // TODO: limits ...
                Set<Group> groups = this._idmClient.findGroupsByNameInGroup(tenant, group.getId(), "", -1);
                Set<PersonUser> users = this._idmClient.findPersonUsersByNameInGroup(tenant, group.getId(), "", -1);
                Set<SolutionUser> solutionUsers = null;
                if ( groupInSystemDomain )
                {
                    solutionUsers = this._idmClient.findSolutionUsersInGroup(systemDomain, group.getName(), "", -1);
                }
                ...
                if ( groups != null )
                {
                    ...
                    {
                        members.add(
                            createGroupMembership(g.getId().getUPN(), ScimGroupMember.Type.GROUP, groupInSystemDomain )
                        );
                    }
                }
                ...
                if ( users != null )
                {
                    ...
                    {
                        members.add(
                            createGroupMembership(user.getId().getUPN(), ScimGroupMember.Type.USER, groupInSystemDomain )
                        );
                    }
                }
        ...
        catch (Exception ex)
        {
            ...
            throw new IllegalStateException(
                String.format("getMembers '%s', '%s' ", groupId, permission.name()), ex);
        }
    }
            ...
            throws ScimResourceNotFoundException
    {
        try
        {
            ...
            if(transitive)
            {
                groups = this._idmClient.findNestedParentGroups(tenant, principalId);
            }
            else
            {
                groups = this._idmClient.findDirectParentGroups(tenant, principalId);
            }
            ...
            if ( groups != null )
            {
                ...
                {
                    members.add(
                        VmidentityScimGroupProvisioning.getScimGroup(g, tenant, systemDomain)
                    );
                }
            }
            ...
            throws ScimResourceNotFoundException, MemberNotFoundException
    {
        ...
        logger.debug(String.format("Scim resource not found '%s' '%s'.", groupId, memberId));
          throw new ScimResourceNotFoundException(
                String.format("Scim resource not found '%s' '%s'.", groupId, memberId) );
    }
    ...
    public ScimGroupMember removeMemberById(String groupId, String memberId)
    {
        try
        {
            ...
            try
            {
                principal = this._idmClient.findGroup(tenant, id);
            }
            catch(InvalidPrincipalException ex)
            {}
            ...
            if ( this._idmClient.removeFromLocalGroup(tenant, id, group.getName()) )
            {
        ...
        catch(InvalidPrincipalException ex)
        {
            ...
            throw new IllegalStateException(
                String.format("addMember '%s', '%s' ", groupId, memberId), ex);
        }
    }
    ...
    public List<ScimGroupMember> removeMembersByGroupId(String groupId) throws ScimResourceNotFoundException
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new IllegalStateException(
                String.format("removeMembersByGroupId '%s' ", groupId), ex);
        }
    }
    ...
    public Set<ScimGroup> removeMembersByMemberId(String memberId) throws ScimResourceNotFoundException
    {
        ...
        try
        {
            ...
            systemDomain = VmidentityUtils.getSystemDomain(tenant, this._idmClient).toUpperCase(Locale.ENGLISH);
        }
        catch (Exception ex)
        {
            logger.debug(String.format("removeMembersByMemberId '%s' ", memberId));
            throw new IllegalStateException(
                String.format("removeMembersByMemberId '%s' ", memberId), ex);
        }
    ...
    private static ScimGroupMember createGroupMembership(String memberId, ScimGroupMember.Type memberType, boolean groupFromSystemDomain) {
